...
import * as clientHooksImport from "virtual:reacticajs:client-hooks";
import * as commonHooksImport from "virtual:reacticajs:common-hooks";
import { detectLanguage } from "./lib/detectBrowserLanguage";

const lastRendered: RenderedStackItem[] =
    (window as any).$reactica$rendered || [];
...
export async function startClient(routes?: Route[]) {
    ...
    if (beforeStartClient) {
        await beforeStartClient($reactica$rootContext);
    }

    const helpers = createLoadHelpers
        ? await createLoadHelpers($reactica$rootContext)
        : {};
    ...
    routes = routes! || window.$reactica$routes!;
    ...
    const stack = (await makeComponentStack({
        ...
        rootContext: $reactica$rootContext,
    ...
    if (wrap) rendered = wrap(rendered, $reactica$rootContext);
