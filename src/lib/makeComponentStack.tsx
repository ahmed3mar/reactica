...
import importers from "virtual:reacticajs:page-imports";
...
export async function makeComponentStack({
    ...
    const content = successfulRender.reduceRight((prev, rendered, i) => {
        ...
        if (import.meta.hot) {
            window.$reactica$reloader[rendered.name || ""] = (newModule) => {
    ...
    return {
        ...
        cacheControl,
        prerender:
            prerender ?? (REACTICA_BUILD_TARGET === "static" && status !== 404),
        crawl: crawl ?? true,
    };
}
...
if (import.meta.hot && !window.$reactica$reloader) {
    window.$reactica$reloader = {};
}